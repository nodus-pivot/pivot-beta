...
const base = {
  ...
  repair_categories: [{ component: "bezel_insert", action: "replace", variant: "ceramic", planned: true, done: true }, { component: "junk", action: "explode" }],
...
describe("toPipelineTicket", () => {
  it("maps rows and jsonb onto the pipeline shape", () => {
    ...
    expect(pt.repair_categories).toEqual([
      { component: "bezel_insert", action: "replace", variant: "ceramic", planned: true, done: true },
      { component: "junk", action: undefined, variant: undefined, planned: undefined, done: undefined },
    ]);
...
describe("stageSummaryLine", () => {
  it("summarizes the repair stage", () => {
    expect(stageSummaryLine("in_repair", base)).toBe("Replace — Bezel/Insert (ceramic) · ? — junk · Insert (from brand); Gasket (bench stock) · Resealed · 1h 30m · Yes");
  });
  it("summarizes received with conditions", () => {
    expect(stageSummaryLine("received", base)).toBe("Sep 1 · Bezel (Cracked) · Bezel/Insert · parts: Insert");
  });
});
